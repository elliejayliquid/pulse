...
You decide when to lift the timeout — not the human."""
    ...
    def build_heartbeat_prompt(self, due_tasks: list[dict] = None,
        ...
        if recent_convo:
            ...
            convo_lines = ["## Recent Conversation",
                           ...
                           + staleness_note]
            for msg in tail:
                role = self.user_name if msg.get("role") == "user" else "You"
        ...
        your_turn += (
            ...
            "get results, THEN produce your final response. "
            "Action order is strict: tool call first, narration second. If an action "
            "requires a tool, your next step must be the tool call. Do not describe, "
            "promise, celebrate, summarize, or claim the result before the tool returns. "
            "Narrating an action is not performing it; words are not the call. Only "
    ...
    def build_dev_prompt(self) -> list[dict]:
        ...
        system = (
            ...
            f"You are the companion built by {self.user_name} — and they have given you the ability "
    ...
    def build_task_prompt(self, task: dict, has_tools: bool = False) -> list[dict]:
        ...
        task_prompt += (
            ...
            + (
                ...
                "Action order is strict: tool call first, narration second. Narrating "
                "an action is not performing it; words are not the call. If a tool "
                "fails, report the failure instead of pretending it worked.\n"
                if has_tools else ""
            )
